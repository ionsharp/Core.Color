...
[Component(0, 1, '%', "L", "Long"), Component(0, 1, '%', "M", "Medium"), Component(0, 1, '%', "S", "Short")]
[Serializable]
...
{    
    ...
    public XYZ ToXYZ(WorkingProfile profile)
        => new(Transform.VonKriesHPEAdjusted.Invert().Value.Multiply(Value));

    /// <summary>(🗸) <see cref="XYZ"/> > <see cref="LMS"/></summary>
    public void FromXYZ(XYZ input, WorkingProfile profile)
        => Value = new(Transform.VonKriesHPEAdjusted.Multiply(input.Value));
